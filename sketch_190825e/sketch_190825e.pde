...
int changeTime = 500; // milliSecond
boolean turnMoment = false;

// MakingCircleSignal valueArea
float lastCircleValue = 0;
float currentCircleValue = 0;
int downValue = 0;
int lasNum = 0;
int curNum = 0;
...
void draw(){
  ...
  drawGrid();
    ...
    if(timeStatus >= changeTime){
      ...
      // Circle Making Place
      lastCircleValue = currentCircleValue;
      currentCircleValue = (lastNumber - (maxValue - minValue)/2) * 0.0001;
    }
    ...
    if(timeStatus >= changeTime - mt || timeStatus <= mt){ //|| 100 > timeStatus){      
      
      if(timeStatus >= changeTime - mt){
        
      }
    
      if(lastNumber == currentNumber){}
      else if(lastNumber < currentNumber){
        interpolateNumber -= abs(curNum-lasNum) * 0.2;
        print(abs(curNum-lasNum) * 0.2 + " | "  );
      }
      else{
        interpolateNumber += abs(curNum-lasNum) * 0.2;
        print(abs(curNum-lasNum) * 0.2 + " | "  );
    ...
    lasNum =  curNum;
    curNum = interpolateNumber;
      
      //println(currentCircleValue + " || " + lastCircleValue);
    println((int)interpolateNumber + " | " + (curNum-lasNum));
